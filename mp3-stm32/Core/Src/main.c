...
void Bouton(void const * argument);
...
//	BSP_SD_Init();
  ...
  /* definition and creation of QueueBP */
  osMessageQDef(QueueBP, 1, uint8_t);
  QueueBPHandle = osMessageCreate(osMessageQ(QueueBP), NULL);
  ...
  /* definition and creation of task_Bouton */
  osThreadDef(task_Bouton, Bouton, osPriorityNormal, 0, 128);
  task_BoutonHandle = osThreadCreate(osThread(task_Bouton), NULL);
...
void Affichage_Pic(void const * argument)
{
  /* USER CODE BEGIN Affichage_Pic */
    ...
    uint8_t action;
    const TickType_t Delai_Atttente = 5000;
    char text[50] = {};
    ...
    /* Initialisation de la carte SD et chargement des metadonnees */
    if (FATFS_LinkDriver(&SD_Driver, SDPath) == 0) {
        if (MP3_Storage_Init() != 0 || MP3_Storage_LoadAllMetadata("/") == 0) {
    		sprintf(text, "Erreur init ou aucun fichier MP3  %lu : %lu",MP3_Storage_Init(),MP3_Storage_LoadAllMetadata("/"));
    		BSP_LCD_DisplayStringAtLine(5, (uint8_t*) text);
            while (1);
        }
    } else {
	...
	for (;;) {
        ...
        MP3FileInfo* file = mp3Infos[counter];

        // Affichage du nom du fichier courant
        BSP_LCD_DisplayStringAtLine(0, (uint8_t*)"Fichier:");
        BSP_LCD_DisplayStringAtLine(1, (uint8_t*)file->filename);

        // Affichage des metadonnees selon le type de tag present
        if (file->tagV1) {
            BSP_LCD_DisplayStringAtLine(3, (uint8_t*)"Titre:");
            BSP_LCD_DisplayStringAtLine(4, (uint8_t*)file->tagV1->title);
            BSP_LCD_DisplayStringAtLine(5, (uint8_t*)"Artiste:");
            BSP_LCD_DisplayStringAtLine(6, (uint8_t*)file->tagV1->artist);
        } else if (file->tagV2) {
            BSP_LCD_DisplayStringAtLine(3, (uint8_t*)"Titre:");
            BSP_LCD_DisplayStringAtLine(4, (uint8_t*)file->tagV2->title);
            BSP_LCD_DisplayStringAtLine(5, (uint8_t*)"Artiste:");
            BSP_LCD_DisplayStringAtLine(6, (uint8_t*)file->tagV2->artist);
        } else {
            BSP_LCD_DisplayStringAtLine(3, (uint8_t*)"Pas de metadonnees.");
        }

        // Gestion du changement de fichier via les boutons (tactile ou GPIO), de manière circulaire
        if (xQueueReceive(QueueBPHandle, &action, Delai_Atttente) == pdPASS) {
            if (action == 1) {
                counter = (counter == 0) ? mp3FileCount - 1 : counter - 1;
            } else if (action == 2) {
                counter = (counter + 1) % mp3FileCount;
            }
        }
	}
...
/* USER CODE BEGIN Header_Bouton */
/**
* @brief Function implementing the task_Bouton thread.
* @param argument: Not used
* @retval None
*/
/* USER CODE END Header_Bouton */
void Bouton(void const * argument)
{
  /* USER CODE BEGIN Bouton */
    GPIO_PinState prev1 = GPIO_PIN_SET;
    GPIO_PinState prev2 = GPIO_PIN_SET;
  /* Infinite loop */
  for(;;)

  {
      GPIO_PinState curr1 = HAL_GPIO_ReadPin(BP1_GPIO_Port, BP1_Pin);
      GPIO_PinState curr2 = HAL_GPIO_ReadPin(BP2_GPIO_Port, BP2_Pin);

      if (prev1 == GPIO_PIN_SET && curr1 == GPIO_PIN_RESET) {
          uint8_t action = 1; // PRECEDENT
          xQueueSend(QueueBPHandle, &action, 0);
      }

      if (prev2 == GPIO_PIN_SET && curr2 == GPIO_PIN_RESET) {
          uint8_t action = 2; // SUIVANT
          xQueueSend(QueueBPHandle, &action, 0);
      }
